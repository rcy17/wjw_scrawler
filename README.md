...
`parser`模块中给出了若干解析函数，由于此时已经解析到一个具体的结点，实际上获取数据应该只需要从该结点标签、文本中获取，目前给出了两个函数，如果遇到不能cover的情况再行添加，万一有需要的话，对于某些特殊网站可以考虑直接把所有数据传到专用解析函数里进行针对性解析。

## 汇报方法

目前尚未完成汇报，新消息直接打印到标准输出上。下一步计划完成smtp部分编写，用邮箱通知的方法推送新消息。
